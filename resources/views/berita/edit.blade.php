...
@section('content')
    <div class="container pt-5">
        ...
        <form action="{{ route('berita.update', ['beritum' => $berita->id]) }}" enctype="multipart/form-data" method="post">
            @csrf
            @method('put')
                            ...
                            <div class="col">
                                <div class="py-3 text-start px-4">
                                    <div class="form-group mb-4">
                                        <label class="mb-2 ms-3"><b>Judul Berita</b></label>
                                        <input type="text" name="judul" value="{{ $berita->judul }}"
                                            ...
                                            placeholder="Tuliskan Judul Berita" required>
                                    </div>
                                    <div class="form-group mb-4">
                                        <label class="mb-2 ms-3"><b>Tempat dan Tanggal</b></label>
                                        <input type="text" name="tempat_tanggal" value="{{ $berita->tempat_tanggal }}"
                                            class="form-control py-2 px-3 rounded-pill border-0"
                                            style="box-shadow: 0px 4px 14px rgba(0, 0, 0, 0.1);"
                                            placeholder="Tuliskan Tempta Tanggal Berita" required>
                                    </div>
                                    <div class="form-group mb-4">
                                        <label class="mb-2 ms-3"><b>Isi Berita</b></label>
                                        <textarea type="text" name="isi" class="form-control py-2 px-3 border-0"
                                            style="box-shadow: 0px 4px 14px rgba(0, 0, 0, 0.1);" placeholder="Masukkan isi berita" rows="10" required>{{ $berita->deskripsi }}</textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
